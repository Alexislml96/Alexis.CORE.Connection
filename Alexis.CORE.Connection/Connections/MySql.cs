...
namespace Alexis.CORE.Connection.Connections
{
    internal class MySql<T> : IConnectionDB<T>
    {
        ...
        public object QueryFirstOrDefaultDapper(TipoDato tipo)
        {
            ...
            {
                _blnPreparado = false;
                return tipo == TipoDato.Numerico ? _clsMySqlConnection.QueryFirstOrDefault<long>(_nombreProcedimiento, _dynParameters, null, _timeOut, _commandType) : _clsMySqlConnection.QueryFirstOrDefault<string>(_nombreProcedimiento, _dynParameters, null, _timeOut, _commandType);
            }
            else
            {
                _blnPreparado = false;
